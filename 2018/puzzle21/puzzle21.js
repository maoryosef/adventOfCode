...
while (REGISTERS[ip] < ops.length) {
	...
	if (REGISTERS[ip] === 29) {
		if (!foundFirst) {
			console.log('first value:', REGISTERS[4]);
			foundFirst = true;
		}

		if (seenValues[REGISTERS[4]]) {
			console.log('last value before loop back:', preVal);
			console.log('first value to loop back:', REGISTERS[4]);
			return;
		}

		seenValues[REGISTERS[4]] = true;
		preVal = REGISTERS[4];
	}
}
