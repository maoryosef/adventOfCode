...
//1 is working should be 29 * 172 = 4988.
//3 is working should be 33 * 948 = 31284
//4 is working should be 37 * 94 = 3478
//5 is working should be 39 * 166 = 6474
//6 is working should be 30 * 38 = 1140
...
let players = [];
let playerId = 0;
const ELF_ATTACK_POWER = parseInt(process.argv[2]) || 4;

console.log('$$$$$ try with ', ELF_ATTACK_POWER);
	...
	for (let y = 0; y < map[x].length; y++) {
		if (map[x][y] === 'G' || map[x][y] === 'E') {
			const p = {
				...
				y,
				hp: 200,
				p: map[x][y] === 'E' ? ELF_ATTACK_POWER : 3
			};
...
while (!endBattle) {
	...
	players.forEach(p => {
		...
		if (nextEnemy) {
			map[nextEnemy.x][nextEnemy.y].hp -= p.p;
	...
	const deadElfs = players.filter(p => p.t === 'E' && p.hp <= 0);

	if (deadElfs.length > 0) {
		console.log('!!!!!!!!!!!!!!!!!! FAILED !!!!!!!!!!!!!!!!!!!');
		return;
	}
