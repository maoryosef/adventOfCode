...
const rules = input.split('\n').reduce((acc: Record<number, Rule[]>, ruleStr) => {
...
function flipHoriz(grid: string[][]) {
	let retVal: string[][] = [];
...
const gridRuleCache: Record<string, string[][]> = {};

function isMatchesRule(grid: string[][], rule: string[][]) {
	let gridClone = grid;
...
function replaceWindow(gridWindow: string[][], ruleSet: Rule[], newInput: string[][], i: number, j: number) {
	let found = false;
...
function replaceGrid(y: number, x: number, grid: string[][], resGrid: string[][]) {
	let retVal = grid;
...
function countPixels(grid: string[][]) {
	let pixelsCount = 0;
	grid.forEach(row => {
		row.forEach(col => {
			if (col === '#') {
