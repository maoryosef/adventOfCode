...
const registers: Record<string, number> = { a: 1, b: 0, c: 0, d: 0, e: 0, f: 0, g: 0, h: 0 };
...
type TokensKeys = keyof typeof TOKENS;
type Token = typeof TOKENS[TokensKeys];
interface OpCache {
	op: Token;
	a: string;
	b: string;
}
const operationsCache: Record<string, OpCache> = {};
...
while (cursor < commands.length) {
	...
	if (!op) {
		...
		op = TOKENS[tokenized[1] as keyof typeof TOKENS];
		a = tokenized[2];
		b = tokenized[3];
