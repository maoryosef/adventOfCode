...
const dec = (reg: number, val: number) => reg - val;
const inc = (reg: number, val: number) => reg + val;
const TOKENS = {
	'inc': inc,
	'dec': dec,
	'>': gt,
...
commands.forEach(cmd => {
	...
	const op = tokenized[2] as keyof typeof TOKENS;
	const val = parseInt(tokenized[3], 10);
	const compare = tokenized[4];
	const operand = tokenized[5] as keyof typeof TOKENS;
	...
	if (TOKENS[operand](registers[compare], compareTo)) {
		registers[registerName] = TOKENS[op](registers[registerName], val) as number;
	}
});
