...
const base = programs.find(program => deps.indexOf(program) === -1)!;
interface ProgramNode {
	node: Pick<Program, 'name' | 'weight'>;
	origWeight: number;
	weightDiff: number;
	fixedWeight: number;
}

const unbalancedNodes: Record<string, ProgramNode> = {};

function calcWeights(node: Program) {
	if (!node.deps) {
		return node.weight;
	}
...
function partition<T>(arr: T[], pred: (t: T) => boolean) {
	const res: T[][] = [[], []];
