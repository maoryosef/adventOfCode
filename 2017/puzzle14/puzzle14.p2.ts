function calculateKnotHash(arrayLength: number, inputLengthsString: string) {
	...
	function calculateSequence(arr: number[]) {
		let retVal = '';
...
const grid: Array<Array<boolean | number>> = [];
...
function calculateGroups(grid: Array<Array<boolean | number>>) {
	function calculateGroupsRec(y: number, x: number, groupNum: number) {
		if (y < 0 ||
			x < 0 ||
			y === grid.length ||
			x === grid[y].length ||
