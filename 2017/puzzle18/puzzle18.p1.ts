...
let lastPlayedSound: number;
let recoveredSound = null;

const set = (a: string, b: string) => { registers[a] = resolveValue(b); };
const add = (a: string, b: string) => { registers[a] = resolveValue(a) + resolveValue(b); };
const mul = (a: string, b: string) => { registers[a] = resolveValue(a) * resolveValue(b); };
const mod = (a: string, b: string) => { registers[a] = resolveValue(a) % resolveValue(b); };
const snd = (a: string) => { playSound(resolveValue(a)); };
const rcv = (a: string) => resolveValue(a) > 0 ? recoverSound() : null;
const jgz = (a: string, b: string) => resolveValue(a) > 0 ? resolveValue(b) : 0;
